...
class IMFRecommender(BaseRecommender):
    def recommend(self, dataset: Dataset, **kwargs) -> RecommendResult:
        ...
        # 正則化パラメータ
        regularization = kwargs.get("regularization", 10)
        # 評価数の閾値
        ...
        model = implicit.als.AlternatingLeastSquares(
            factors=factors, regularization=regularization, iterations=n_epochs, calculate_training_loss=True, random_state=1
        )

        # 学習
        model.fit(movielens_matrix.T.tocsr(), show_progress = False)

        # 推薦
        # print(type(movielens_matrix.tocsr()))
